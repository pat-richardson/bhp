...
class NetCat:
    ...
    def send(self):
        self.socket.connect((self.args.target, self.args.post))
        if self.buffer:
...
def execute(cmd):
    ...
    # runs command on local OS and returns the output
...
def main():
    help_text = '''Example:
    netcat.py -t 192.168.1.108 -p 5555 -l -c # initialize command shell    
    netcat.py -t 192.168.1.108 -p 5555 -l -u=mytest.txt # upload received file and write to destination
